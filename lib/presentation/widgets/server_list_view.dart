...
class _ServerListViewState extends State<ServerListView> {
  ...
  Widget build(BuildContext context) {
    ...
    final bookmarks = vpn.bookmarkedServers;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        if (bookmarks.isNotEmpty)
          _buildBookmarksSection(context, vpn, isConnected, bookmarks),
        for (int i = 0; i < countries.length; i++) ...[
          ...
          if (_expanded.contains(countries[i]))
            for (final server in grouped[countries[i]]!)
              _buildServerTile(context, vpn, server),
        ],
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        Padding(
          ...
          child: Row(
            children: [
              const Icon(Icons.bookmark_rounded, size: 18, color: AppColors.accent),
        ...
        for (final server in bookmarks) _buildServerTile(context, vpn, server),
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              _CountBadge(label: '${servers.length}', color: AppColors.accent),
              const SizedBox(width: 6),
              Icon(
                isExpanded
  ...
  Widget _buildServerTile(BuildContext context, VpnState vpn, VpnServer server) {
    final bool isSelected =
        !vpn.useCustomConfig && vpn.selectedServer?.id == server.id;
    final bool isBookmarked = vpn.isBookmarked(server);
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Text(
                          'Node #${server.id}',
                          style: TextStyle(fontSize: 10, color: Colors.cyan[200]),
                        ),
                        const SizedBox(width: 8),
                        Container(
                          width: 3,
